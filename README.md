...
## واجهة التداول وإضافة سهم

- الواجهة المبسطة متاحة في `/trading-room`: تحليل سهم واحد، أفضل ثلاث فرص محفوظة، بطاقة نتيجة واحدة، أمر Limit قابل للتعديل، ومحفظة مختصرة.
- لا يُنشأ `TradeIntent` إلا بعد فرصة سهم صالحة وBid/Ask حديثين وسبريد مقبول وسياق جلسة صالح. اختيار عقد الأوبشن يأتي بعد بوابة السهم فقط ويبقى معزولًا خلف `OPTIONS_ENABLED`.
- `TradeIntent` يحفظ وقت الإنشاء وآخر وقت للدخول وفترة الاحتفاظ والخروج المتوقع والخروج الإلزامي والجلسة وعمر الإشارة. انتهاء وقت الدخول يمنع التنفيذ ويطلب إعادة التحليل.
- Paper Mode هو الافتراضي. `TRADING_CONFIRM_MODE_ENABLED=false` افتراضيًا، ولا يوجد Full Auto.
- إضافة Chrome المحلية موجودة في `browser-extension/`. راجع [تعليمات التثبيت والأمان](browser-extension/README-AR.md) قبل التجربة.
- تكامل سهم يعتمد على DOM المرئي وليس API موثقًا؛ لذلك يرفض التنفيذ إذا لم يجد الحقول الأصلية للكمية وLimit وTake Profit وStop Loss وزر الإرسال بصورة واضحة.
