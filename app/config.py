...
class Settings(BaseSettings):
    ...
    spx_background_refresh_seconds: int = Field(
        default=30, ge=15, le=300, alias="SPX_BACKGROUND_REFRESH_SECONDS"
    )
    spx_observation_min_spacing_seconds: int = Field(
        default=25, ge=10, le=300, alias="SPX_OBSERVATION_MIN_SPACING_SECONDS"
    )
    spx_direction_max_age_seconds: int = Field(
        default=90, ge=30, le=300, alias="SPX_DIRECTION_MAX_AGE_SECONDS"
    )
    spx_ai_review_interval_seconds: int = Field(
        default=600, ge=300, le=3600, alias="SPX_AI_REVIEW_INTERVAL_SECONDS"
    )
    spx_ai_review_max_age_seconds: int = Field(
        default=900, ge=300, le=3600, alias="SPX_AI_REVIEW_MAX_AGE_SECONDS"
    )
    ...
    min_risk_reward: float = Field(default=1.25, alias="MIN_RISK_REWARD")
    min_strategy_match_pct: int = Field(default=50, alias="MIN_STRATEGY_MATCH_PCT")
    speculative_small_cap_enabled: bool = Field(default=True, alias="SPECULATIVE_SMALL_CAP_ENABLED")
    speculative_min_price: float = Field(default=0.5, alias="SPECULATIVE_MIN_PRICE")
    speculative_max_price: float = Field(default=8.0, alias="SPECULATIVE_MAX_PRICE")
    speculative_max_spread_pct: float = Field(default=4.0, alias="SPECULATIVE_MAX_SPREAD_PCT")
    speculative_min_strategy_match_pct: int = Field(default=40, alias="SPECULATIVE_MIN_STRATEGY_MATCH_PCT")
    speculative_min_risk_reward: float = Field(default=1.15, alias="SPECULATIVE_MIN_RISK_REWARD")
    speculative_max_spread_to_target_pct: float = Field(default=40.0, alias="SPECULATIVE_MAX_SPREAD_TO_TARGET_PCT")
